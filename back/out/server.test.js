...
beforeEach(async () => {
    ...
    for (let { id, make, color, manufacture_date, notes } of widgets) {
        await db.run("INSERT INTO widgets(id, make, color, manufacture_date, notes) VALUES(?, ?, ?, ?, ?)", [id, make, color, manufacture_date, notes]);
    }
});
afterEach(async () => {
...
test("POST /widgets valid body with notes creates widget", async () => {
    ...
    let dbWidget = await (await getDB()).get("SELECT * FROM widgets WHERE id = ?", [id]);
